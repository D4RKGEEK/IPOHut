...
async function fetchAPI<T>(endpoint: string, params?: Record<string, string | number | boolean>): Promise<APIResponse<T>> {
...
export async function fetchGains(params?: {
  ...
  return fetchAPI<IPOGain[]>("/api/gains", params);
}
...
export async function fetchStatus(params: {
  ...
  return fetchAPI<IPOStatus[]>("/api/status", params);
}
...
export async function fetchNews(params?: {
  ...
  return fetchAPI<IPONews[]>("/api/news", params);
}
...
export async function fetchCalendar(params?: {
  ...
  return fetchAPI<IPOCalendar[]>("/api/calendar", params);
}

// Detail API
export async function fetchIPODetail(slug: string): Promise<APIResponse<IPODetail>> {
  return fetchAPI<IPODetail>(`/api/details/${slug}`);
}
